...
def search(query):
    ...
    headers = {
        'X-API-KEY': 'fb0df7ac2331bdc47c2052fb16e078c396360aab',
...
def research(query):
    ...
    user_proxy.send(
        "Give me the content that just generated again, return ONLY the report & reference links", researcher)
...
def write_content(research_material, topic):
    editor = autogen.AssistantAgent(
        name="editor",
        system_message="You are a senior editor of a content writing and case study, you will define the structure of a content that is presentable to show the client based on material provided by the researcher, and give it to the writer to write the report about the research",
        llm_config={"config_list": config_list},
    )

    writer = autogen.AssistantAgent(
        name="writer",
        system_message="You are a professional content writer who is writing content in a presentable format to the client based on the research, you will write a report based on the structured provided by the editor, and feedback from reviewer; After 2 rounds of content iteration, add TERMINATE to the end of the message",
        llm_config={"config_list": config_list},
    )

    reviewer = autogen.AssistantAgent(
        name="reviewer",
        system_message="You are a world class report content critic, you will review & critic the written report and provide feedback to writer.After 2 rounds of content iteration, add TERMINATE to the end of the message",
        llm_config={"config_list": config_list},
    )
...
user_proxy.initiate_chat(
    writing_assistant, message="Write me blog post about how to stop an AI from working.")